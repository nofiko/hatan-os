...
mkdir -p "$ROOT/ui/shell/assets/audio" "$ROOT/ui/installer/assets" "$ROOT/ui/installer/css" \
    "$ROOT/ui/boot/assets/audio" "$ROOT/themes/plymouth"
...
if [[ -n "$LOGO_SRC" ]]; then
    ...
    cp "$LOGO_SRC" "$ROOT/ui/boot/assets/logo.png" 2>/dev/null || true
...
cp "$ROOT/themes/custom/theme.css" "$ROOT/ui/boot/css/theme.css" 2>/dev/null || true
...
for f in startup-sound.mp3 welcom.mp3 select.mp3 DIS.mp3; do
    ...
    [[ -f "$ROOT/themes/audio/$f" ]] && cp "$ROOT/themes/audio/$f" "$ROOT/ui/boot/assets/audio/$f" 2>/dev/null || true
done
